...
if ARGV[0].nil? || ARGV[1].nil?
  puts "参数错误！接收两个参数，开始时间和结束时间。"
  exit(0)
end

start_time = Time.now
s_time = Time.parse(ARGV[0]).beginning_of_day.utc
e_time = Time.parse(ARGV[1]).end_of_day.utc

condition = {created_at:s_time..e_time}

#*先创建Delivery
ds = Old::ODelivery.where(condition).all
...
puts "======================"
puts "共耗时#{(Time.now - start_time).to_s}秒,#{distance_of_time_in_words(Time.now - start_time)}"
puts "======================"
