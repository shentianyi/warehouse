module FileHandler
  module Excel
    class DeliveryHandler<Base
      ...
      JIAXUAN_HEADERS=[
          :package_id, :custom_name, :batch, :hem_part_nr, :part_desc, :qty, :qty_unit,
          :bundle_qty, :destination_address, :delivery_date, :arrive_date, :carrier
      ]

      JIAXUAN_ZH_HEADERS=[
          "唯一码", "客户名", "申请单号", "产品", "产品名", "发货数量", "存货单位",
          "箱数", "送货地址", "装运日期", "要求到货日期", "承运人"
      ]

      def self.calc_qty qty, qty_unit, bundle_qty
        (qty.to_f * (qty_unit=='KPC' ? 1000 : 1))/bundle_qty.to_i
      end

      def self.check_qty qty, qty_unit, bundle_qty
        (qty * (qty_unit=='KPC' ? 1000 : 1))/bundle_qty
      end
        ...
        if validate_msg.result
          # begin
            ActiveRecord::Base.transaction do
              ...
              dlc = delivery.logistics_containers.build(source_location_id: source.id, des_location_id: destination.id, user_id: user.id, remark: '海尔曼发运数据', state: MovableState::WAY)
              ...
              2.upto(book.last_row) do |line|
                ...
                JIAXUAN_HEADERS.each_with_index do |k, i|
                  ...
                  if [:package_id, :hem_part_nr].include?(k)
                    row[k]=row[k].sub(/\.0/, '')
                  end
                end

                part = Part.find_by_nr(row[:hem_part_nr])
                if row[:package_id].include?('-')
                  ###################################多个 唯一码######################################################
                  row[:package_id].split('-').first.upto(row[:package_id].split('-').last) do |package_id|
                    if plc = LogisticsContainer.find_latest_by_container_id(package_id)
                      ...
                      package = Package.create({
                                                   ...
                                                   part_id: part.id,
                                                   user_id: user.id,
                                                   quantity: calc_qty(row[:qty], row[:qty_unit], row[:bundle_qty]),
                                                   state: PackageState::WAY,
                                                   extra_carrier: row[:carrier],
                                                   extra_custom_name: row[:custom_name],
                                                   extra_destination_address: row[:destination_address],
                                                   extra_delivery_date: row[:delivery_date],
                                                   extra_arrive_date: row[:arrive_date],
      ...
      def self.validate_row(row)
        ...
        if row[:hem_part_nr].blank?
          msg.contents<<"零件号:#{row[:hem_part_nr]}不能为空"
        else
          if Part.find_by_nr(row[:hem_part_nr]).blank?
            msg.contents<<"零件号:#{row[:forklift_id]}不存在"
          end
        end
        ...
        if row[:package_id].blank?
          ...
          if row[:package_id].include?('-')
            ...
            unless (p_ids - NStorage.all.pluck(:packageId)) == p_ids
              msg.contents<<"唯一码:#{p_ids - (p_ids - NStorage.all.pluck(:packageId))}已存在"
            end
            package_count = p_ids.count
          else
          ...
          puts package_count
          puts row[:bundle_qty]
          unless package_count == row[:bundle_qty].to_i
            msg.contents<<"唯一码数量和箱数量不匹配"
          end
        end
