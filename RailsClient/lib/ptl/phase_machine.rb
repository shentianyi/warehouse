            ...
            case job.to_state
              ...
              when Node::URGENT_ORDERED
                ...
                if Ptl::State::PtlOrderState.do_order?(job.ptl_job.order_state)
                  result=LedService.create_stockout_list(node.id, job.size, job.urgent_size)
                  if result
                    job.ptl_job.update_attributes(order_state: Ptl::State::PtlOrderState::SUCCESS)
                  end
                end
                if result
                  node.set_display(displays[0]+job.urgent_size, displays[1]+job.urgent_size+job.size)
                end
